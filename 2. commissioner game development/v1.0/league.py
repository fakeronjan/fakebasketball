...
class League:
    ...
    def _create_teams(self, selected: Optional[list] = None) -> list[Team]:
        ...
        # Pre-compute initial qualities based on spread mode
        lo, hi = self.cfg.min_quality, self.cfg.max_quality
        mid = (lo + hi) / 2.0
        n = len(starters)
        mode = self.cfg.initial_quality_mode
        if mode == "uniform":
            initial_qualities = [mid] * n
        elif mode == "haves_havenots":
            haves = n // 2
            have_q = [random.uniform(mid, hi) for _ in range(haves)]
            not_q  = [random.uniform(lo, mid) for _ in range(n - haves)]
            initial_qualities = have_q + not_q
            random.shuffle(initial_qualities)
        else:  # moderate (default)
            initial_qualities = [random.uniform(lo, hi) for _ in range(n)]

        teams = []
        for i, (f, q) in enumerate(zip(starters, initial_qualities), 1):
            team = Team(
                i,
                q,
                f,
    ...
    # Quality reset fractions for playoff teams — expressed as a fraction of
    # (max_quality - min_quality) above min_quality.  This makes the table
    # independent of the chosen quality range so all three variance presets
    # produce the same *relative* pecking order.
    # Key: (rounds_from_finals, series_length) where rounds_from_finals=0 = finals loser.
    _PLAYOFF_QUALITY_FRACS = {
        (0, 4): 0.933, (0, 5): 0.933, (0, 6): 0.967, (0, 7): 0.967,
        (1, 4): 0.767, (1, 5): 0.800, (1, 6): 0.833, (1, 7): 0.867,
        (2, 4): 0.600, (2, 5): 0.633, (2, 6): 0.667, (2, 7): 0.733,
        (3, 4): 0.333, (3, 5): 0.367, (3, 6): 0.400, (3, 7): 0.433,
    }
    ...
    def _offseason_adjustments(self, season: Season) -> None:
        ...
        q_range = self.cfg.max_quality - self.cfg.min_quality

        # Consume one season of any active talent investment
        talent_boost = 0.0
        if self._talent_boost_seasons_left > 0:
            talent_boost = self._talent_boost_delta
        ...
        for team in self.teams:
            if team is season.champion:
                ...
                frac = self._PLAYOFF_QUALITY_FRACS.get(key, 0.333)
                team.quality = self.cfg.min_quality + frac * q_range
            else:
                mu = self._market_bias(team) + talent_boost
    ...
    def start_talent_investment(self, delta: float, seasons: int) -> None:
        """Add a talent boost that lifts non-playoff team offseason quality mean.
        Stacks additively on duration with any existing investment."""
    ...
    def _add_expansion_team(self, franchise: Franchise, joined_season: int,
                            starting_quality: float = None) -> Team:
        """Create a new expansion team and add it to the league."""
        is_secondary = franchise.secondary
        quality = starting_quality if starting_quality is not None else self.cfg.min_quality
