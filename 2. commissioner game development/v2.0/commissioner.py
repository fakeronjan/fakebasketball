...
""")
  ...
  {BOLD}Column key:{RESET}
    ...
    {CYAN}ORtg{RESET}      Offensive rating — pts scored per 100 possessions
    {CYAN}DRtg{RESET}      Defensive rating — pts allowed per 100 possessions (lower = better)
    {CYAN}Pace{RESET}      Possessions per team per game
    {CYAN}3pt%{RESET}      Share of scoring possessions that go for 3
""")
        teams = self.league.teams
        cfg   = self.league.cfg
        max_fb = max(_fan_base(t) for t in teams)
        teams_sorted = sorted(teams, key=lambda t: -(t.ortg - t.drtg))
        print(f"  {'Team':<30}  {'Fan Base':<22}  {'ORtg':>4}  {'DRtg':>4}  {'Pace':>4}  3pt%")
        divider()
        for t in teams_sorted:
            fb      = _fan_base(t)
            fb_str  = _fan_base_bar(fb, max_fb, 12)
            name_padded = f"{t.name:<30}"
            print(f"  {BOLD}{name_padded}{RESET}  {fb_str}  {t.ortg:>4.1f}  {t.drtg:>4.1f}  {t.pace:>4.0f}  {t.style_3pt:>4.0%}")
    ...
    def _offer_round_interventions(
        ...
        while True:
            ...
            # Show matchups
            print(f"\n  {'#':>2}  {'Seed 1':<26}  {'Seed 2':<26}  {'NET':>4}v{'':<4}  Status")
            divider()
            ...
            o1, d1, p1, s3_1 = season._start_ratings.get(s1, (s1.ortg, s1.drtg, s1.pace, s1.style_3pt))
            o2, d2, p2, s3_2 = season._start_ratings.get(s2, (s2.ortg, s2.drtg, s2.pace, s2.style_3pt))
            fb1 = _fan_base(s1);  fb2 = _fan_base(s2)

            print(f"\n  {'':4}  {'Team':<28}  {'ORtg':>4}  {'DRtg':>4}  {'Pace':>4}  {'Fan Base':>9}")
            divider()
            print(f"  {CYAN}[1]{RESET}  {s1.franchise_at(sn).name:<28}  {o1:>4.0f}  {d1:>4.0f}"
                  f"  {p1:>4.0f}  {fb1:>7.1f}M")
            print(f"  {CYAN}[2]{RESET}  {s2.franchise_at(sn).name:<28}  {o2:>4.0f}  {d2:>4.0f}"
                  f"  {p2:>4.0f}  {fb2:>7.1f}M")
            print()
    ...
    def _play_series_with_bonus(
        ...
        for game_num in range(cfg.series_length):
            ...
            ph = _p_score(home, away, cfg, bonus=home_bonus)
            pa = _p_score(away, home, cfg, bonus=away_bonus)
    ...
    def _show_team_history(self, season: Season):
        """Team-level history selector. Each team object gets its own entry."""
        league = self.league
        seasons = league.seasons

        # Build seasons-per-team lookup
        team_seasons: dict = {t: [s for s in seasons if t in s.teams] for t in league.teams}

        # Sort by fan base descending
        team_list = sorted(league.teams, key=lambda t: -_fan_base(t))

        while True:
            clear()
            header("TEAM HISTORY", "Select a team")
            print(f"\n  {'#':>3}  {'Team':<26} {'Seasons':>7}  {'Titles':>6}  Notes")
            divider()
            for i, t in enumerate(team_list, 1):
                played = team_seasons[t]
                titles = sum(1 for s in played if s.champion is t)
                title_str = f"{GOLD}{titles}★{RESET}" if titles else f"{MUTED} —{RESET}"
                relocated = len(t.franchise_history) > 1
                note = f"{MUTED}relocated {len(t.franchise_history)-1}x{RESET}" if relocated else ""
                print(f"  {i:>3}. {t.franchise.name:<26} {len(played):>7}  {title_str:>6}  {note}")
            print(f"\n  {CYAN}[0]{RESET} {MUTED}Back{RESET}")

            raw = prompt("Team number (Enter to go back):")
            if raw == "":
                break
            if not raw.isdigit():
                continue
            ...
            if val == 0:
                break
            if 1 <= val <= len(team_list):
                t = team_list[val - 1]
                self._show_team_detail(t, team_seasons[t])

    def _show_team_detail(self, team, entries: list):
        """Season-by-season history for one team."""
        PAGE = 20
        ...
        while True:
            ...
            header("TEAM HISTORY", f"{team.franchise.name}{champ_str}")

            # Show franchise name changes (relocations)
            if len(team.franchise_history) > 1:
                parts = [f"{MUTED}S{sn}{RESET} {f.name}" for sn, f in team.franchise_history]
                print(f"\n  Franchise history: {' → '.join(parts)}")

            print(f"\n  {'S':>3}  {'Name':<22} {'Record':<14} {'Pos':>4}  "
    ...
    def _do_invest_in_talent(self, season: Season):
        ...
        boost = round(0.04 * (cfg.ortg_max - cfg.ortg_min), 3)

        print(f"  Effect: non-playoff teams gain a +{boost:.1f} pts ORtg mean bonus each")
...
""")
    ...
    def _handle_rule_change_offer(self, season: Season):
        ...
        print(f"  The league has been in a prolonged {RED}{direction}{RESET} era "
              ...
              f"({MUTED}{champ_style} style{RESET})\n"
        ...
        if action == "favor":
            ...
            sign = -1 if champ.style_3pt > 0.25 else 1   # oppose champ's style
            league.league_meta = max(-cfg.meta_max,
            ...
            season.meta_shock = True
            self._rule_change_cooldown = 10
            counter_dir = "interior" if champ.style_3pt > 0.25 else "3pt-heavy"
