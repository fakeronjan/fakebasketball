...
def _p_score(attacker: Team, defender: Team, cfg: Config, bonus: float = 0.0) -> float:
    """Probability of scoring on a given possession.

    Formula: (attacker.ortg + defender.drtg - baseline) / (100 × E[pts|scored])
    - High attacker ORtg → more scoring.
    - Low defender DRtg (good defense) → less scoring.
    - Both effects are additive from the league average baseline.
    """
    e_scored = 2.0 + attacker.style_3pt
    raw = (attacker.ortg + defender.drtg - cfg.ortg_baseline) / (100.0 * e_scored) + bonus
...
def play_game(home: Team, away: Team, cfg: Config, home_advantage=None,
    ...
    ph   = _p_score(home, away, cfg, bonus=ha)
    pa   = _p_score(away, home, cfg)
