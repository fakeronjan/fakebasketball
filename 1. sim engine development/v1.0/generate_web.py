...
def build_season_rows(league: League) -> list[dict]:
    ...
    for s in league.seasons:
        ...
        rs_leader = stand[0]
        rs_2nd = stand[1]
        repeat = True if champ is prev_champ else False

        # Clinching game score — champion score first
        clincher = s.playoff_rounds[-1][0].games[-1]
        if clincher.winner is champ:
            clinch_str = f"{clincher.home_score}–{clincher.away_score}" if clincher.home is champ else f"{clincher.away_score}–{clincher.home_score}"
        else:
            clinch_str = f"{clincher.home_score}–{clincher.away_score}"

        rows.append({
            "season": s.number,
            "champ": nm(champ, s.number),
            "champ_seed": _playoff_seed(s, champ),
            "repeat": repeat,
            "finalist": nm(finalist, s.number),
            ...
            "rs_2nd": nm(rs_2nd, s.number),
            "rs_2nd_w": s.reg_wins(rs_2nd),
            "rs_2nd_l": s.reg_losses(rs_2nd),
        })
        prev_champ = champ
    return rows
...
def season_table_html(rows: list[dict]) -> str:
    ...
    for r in rows:
      ...
      <tr>
        <td>{r["season"]}</td>
        <td class="champ-cell">{repeat}{r["champ"]} <span class="seed">({r["champ_seed"]})</span></td>
        <td>{r["finalist"]} <span class="seed">({r["finalist_seed"]})</span></td>
        <td class="fin-score">{r["fin"]}</td>
        <td class="fin-score">{r["clinch"]}</td>
        <td>{r["rs_leader"]} {r["rs_leader_w"]}–{r["rs_leader_l"]}</td>
        <td>{r["rs_2nd"]} {r["rs_2nd_w"]}–{r["rs_2nd_l"]}</td>
      </tr>""")
...
<div class="hero">
  ...
  <p class="subtitle">A 100-season simulation recreating a program originally written in Visual Basic — roughly 30 years ago.</p>
...
<footer>
  ...
  <p style="margin-top:0.4rem">Recreating a Visual Basic program from ~1995, one season at a time.</p>
</footer>
...
if __name__ == "__main__":
    random.seed(SEED)
    cfg = Config(num_seasons=NUM_SEASONS, strength_delta=0.03)
    league = League(cfg)
    league.simulate()
