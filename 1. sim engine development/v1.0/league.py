...
class League:
    ...
    def _create_teams(self) -> list[Team]:
        ...
        return [
            Team(i, random.uniform(self.cfg.min_strength, self.cfg.max_strength), f)
    ...
    # Strength table: (rounds_from_finals, series_length) -> starting strength next season
    ...
    def _offseason_adjustments(self, season: Season) -> None:
        """Reset team strengths based on playoff finish; randomise non-playoff teams."""
        ...
        for team in self.teams:
            if team is season.champion:
                team.strength = self.cfg.max_strength
            elif team in playoff_exit:
                key = playoff_exit[team]
                team.strength = self._PLAYOFF_STRENGTH.get(key, self.cfg.max_strength - 0.20)
            else:
                mu = self._market_bias(team)
                adjustment = random.gauss(mu, self.cfg.offseason_sigma)
                team.strength = max(
                    self.cfg.min_strength,
                    min(self.cfg.max_strength, team.strength + adjustment),
                )
