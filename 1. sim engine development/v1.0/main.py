...
def print_standings(season: Season) -> None:
    print(f"\n  {'#':<4} {'Team':<24} {'W':>4} {'L':>4}  {'Pct':>5}  {'Str':>5}")
    print(f"  {_bar(52)}")
    for rank, team in enumerate(season.regular_season_standings, 1):
        marker = "*" if rank <= season.cfg.playoff_teams else " "
        print(
            f"  {marker}{rank:<3} {team.name:<24} "
            f"{season.reg_wins(team):>4} {season.reg_losses(team):>4}  "
            f"{season.reg_win_pct(team):.3f}  {team.strength:.3f}"
        )


def print_playoffs(season: Season) -> None:
    bracket_sizes = [season.cfg.playoff_teams // (2 ** r) for r in range(len(season.playoff_rounds))]
...
def print_season(season: Season, relocations: list[tuple[int, str, str]]) -> None:
    ...
    # Show any relocations that happened after this season
    moves = [(o, n) for (s, o, n) in relocations if s == season.number]
    if moves:
        print(f"\n  Offseason moves:")
...
def print_history(league: League) -> None:
    ...
    print(f"{'═' * 62}")
    print(f"\n  {'Season':<8} {'Champion':<26} {'Str':>5}")
    print(f"  {_bar(42)}")
    for s in league.seasons:
        nm = s.champion.franchise_at(s.number).name
        print(f"  {s.number:<8} {nm:<26} {s.champion.strength:.3f}")
