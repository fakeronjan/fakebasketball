...
def play_game(home: Team, away: Team, cfg: Config, home_advantage=None) -> GameResult:
    """Simulate one game. Updates team strengths; does NOT track win/loss records."""
    eff_home = home.strength + (home_advantage if home_advantage is not None else cfg.home_advantage)

    home_score = sum(_sim_possession(eff_home) for _ in range(cfg.possessions))
    away_score = sum(_sim_possession(away.strength) for _ in range(cfg.possessions))

    # No ties — keep simulating extra possessions until decided
    while home_score == away_score:
        home_score += _sim_possession(eff_home)
        away_score += _sim_possession(away.strength)

    result = GameResult(home, away, home_score, away_score)

    result.winner.strength = min(result.winner.strength + cfg.strength_delta, cfg.max_strength)
    result.loser.strength = max(result.loser.strength - cfg.strength_delta, cfg.min_strength)
