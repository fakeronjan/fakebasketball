...
def _possessions(cfg: Config, league_meta: float) -> int:
    """Possession count scales with league era: offensive eras = more possessions."""
    return max(50, round(cfg.possessions + league_meta * cfg.possession_scale))


def play_game(home: Team, away: Team, cfg: Config, home_advantage=None, league_meta: float = 0.0) -> GameResult:
    ...
    poss = _possessions(cfg, league_meta)
...
def play_series(seed1: Team, seed2: Team, cfg: Config, home_advantage=None, league_meta: float = 0.0, seed_bonus: float = 0.0) -> tuple[Team, list[GameResult]]:
