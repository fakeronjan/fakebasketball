    ...
    def play_playoffs(self) -> None:
        ...
        while len(bracket) > 1:
            ...
            for i in range(len(bracket) // 2):
                ...
                winner, games = play_series(s1, s2, self.cfg, home_advantage=self.cfg.playoff_home_advantage, league_meta=self.league_meta, seed_bonus=self.cfg.playoff_seed_bonus)
                for g in games:
                    self._record(g)
