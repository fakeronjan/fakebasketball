...
def build_alltime(league: League) -> dict:
    ...
    # Series length counts by round (index 0 = quarterfinals, -1 = finals)
    num_rounds = league.cfg.playoff_teams.bit_length() - 1  # log2(playoff_teams)
    series_counts_by_round = [{} for _ in range(num_rounds)]
    for s in seasons:
...
def build_team_histories(league: League) -> list:
    ...
    num_rounds = len(league.seasons[0].playoff_rounds)
    round_labels = ["QF", "SF", "Finals"]

    # Group (season, team) pairs by the franchise name active that season.
    # This means relocations split histories; a team returning to a city merges them.
                ...
                for round_idx, rnd in enumerate(s.playoff_rounds):
                    for sr in rnd:
                        if sr.seed1 is team or sr.seed2 is team:
                            ...
                            label = round_labels[round_idx] if round_idx < len(round_labels) else f"R{round_idx+1}"
                            matchups.append({
                                "label": label,
                                "won": won,
                                "tw": tw, "tl": tl,
...
def build_seed_grid(league: League) -> dict:
    ...
    n_seeds = league.cfg.playoff_teams
    for s in league.seasons:
...
def rivalries_html(data: list, label: str) -> str:
    ...
    for a, b, meetings, record in data:
        ...
        rows.append(
            f'<tr>'
            f'<td>{a} <span style="color:var(--muted);font-size:0.82em">({wa})</span></td>'
            f'<td style="color:var(--muted);padding:0 0.4rem">vs</td>'
            f'<td>{b} <span style="color:var(--muted);font-size:0.82em">({wb})</span></td>'
...
<section id="seed-grid">
  <div class="inner">
    ...
    {(lambda rounds: f'''<table class="data-table">
      <thead><tr>
        <th>Round</th>
      ...
      for name, counts in zip(["Quarterfinals","Semifinals","Finals"], rounds) if counts
      )}</tbody>
    </table>''')(alltime["series_counts_by_round"])}
  </div>
</section>
