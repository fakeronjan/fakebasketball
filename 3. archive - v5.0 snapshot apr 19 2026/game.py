...
def _pick_shooter(team: Team, cfg: Config) -> tuple[Player | None, int]:
...
def _pick_defender(defense: Team, shooter: Player | None) -> Player | None:
    """Pick the primary defender, preferring a positional match to the shooter."""
    defenders = [p for p in defense.roster if p is not None]
    if not defenders:
        return None
...
def _compute_make_pct(
    ...
    if shooter:
        off_adj = (shooter.ortg_contrib * shooter.happiness_mult
                   * off_chemistry * (1.0 + league_meta) / ortg_scale)

    def_adj = 0.0
    if defender and zone != ZONE_FT:
        def_adj = (defender.drtg_contrib
                   * def_chemistry * (1.0 - league_meta) / ortg_scale)
...
def _sim_possession(
    ...
    """
    shooter, _  = _pick_shooter(offense, cfg)
    zone        = _pick_zone(shooter)
    defender    = _pick_defender(defense, shooter)
    ...
    if zone == ZONE_FT:
        pct = _compute_make_pct(shooter, None, ZONE_FT,
                                ...
                                league_meta, effective_bonus, ortg_scale)
