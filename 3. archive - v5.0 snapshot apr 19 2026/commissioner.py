...
class CommissionerGame:
    ...
    def _show_wip_status(self):
        clear()
        header("FAKE BASKETBALL", "Commissioner Mode  ·  v0.1  ·  Work in Progress")
        print(f"""
...
""")
        ...
        todos = [
            "Rival league  (breakaway threat, poaching owners, legitimacy war)",
...
""")
  ...
  {GREEN}[A] Endorse ($20M owner investment){RESET}  Talent boost next season. Owner {GREEN}+happy{RESET}.
  {GOLD}[C] Encourage without endorsing ($10M){RESET}  Smaller boost. Modest happiness.
  {RED}[R] Decline{RESET}                            Owner {GOLD}frustrated{RESET}.
""")
            while True:
                r = prompt("Decision [A/C/R]:").strip().upper()
                if r in ("A","C","R"): break
            if r == "A":
  ...
  {GREEN}[A] Agree quietly{RESET}  Owner {GREEN}+confidence{RESET}. Legitimacy {RED}−5%{RESET}.
  {RED}[R] Decline{RESET}        Owner {GOLD}mildly frustrated{RESET}. No cost.
""")
            while True:
                r = prompt("Decision [A/R]:").strip().upper()
                if r in ("A","R"): break
            if r == "A":
  ...
  {GREEN}[A] Approve{RESET}  Owner revenue {GREEN}+${deal}M{RESET}. Arena renamed.
  {RED}[R] Decline{RESET}  Owner {GOLD}frustrated{RESET}.
""")
            while True:
                r = prompt("Decision [A/R]:").strip().upper()
  ...
  {GREEN}[A] Fund it ($8M){RESET}   Popularity {GREEN}+6%{RESET} · Engagement {GREEN}+3%{RESET} · Owner {GREEN}happy{RESET}.
  {GOLD}[C] Half-fund ($4M){RESET}  Smaller boost.
  {RED}[R] Decline{RESET}          Owner {GOLD}disappointed{RESET}.
""")
            while True:
                r = prompt("Decision [A/C/R]:").strip().upper()
                if r in ("A","C","R"): break
  ...
  {GREEN}[A] Begin sale process{RESET}         Ownership transition initiated.
  {GOLD}[C] Talk them out of it ($5M){RESET}   Show of confidence. Owner reconsiders.
  {RED}[R] Acknowledge, delay{RESET}           Noted. Revisit next season.
""")
            while True:
                r = prompt("Decision [A/C/R]:").strip().upper()
                if r in ("A","C","R"): break
  ...
  {GREEN}[A] Approve publicly{RESET}   Revenue {GREEN}+${deal}M{RESET}. Legitimacy {RED}−10%{RESET}. Players & union notice.
  {GOLD}[C] Approve quietly{RESET}    Revenue {GOLD}+${deal//2}M{RESET}. Legitimacy {RED}−5%{RESET}. Lower visibility.
  {RED}[R] Reject{RESET}             Owner {RED}very unhappy{RESET}. League stays clean.
""")
            while True:
                r = prompt("Decision [A/C/R]:").strip().upper()
                if r in ("A","C","R"): break
  ...
  {GREEN}[A] Approve{RESET}  Revenue {GREEN}+${sdeal}M{RESET}. Legitimacy {RED}−{int(shlegit*100)}%{RESET}.
  {RED}[R] Reject{RESET}   Owner {GOLD}frustrated{RESET}.
""")
            while True:
                r = prompt("Decision [A/R]:").strip().upper()
                if r in ("A","R"): break
  ...
  {GREEN}[A] Endorse publicly{RESET}   Treasury {GREEN}+$10M{RESET} (subsidy flows to league). Legitimacy {RED}−5%{RESET}.
  {GOLD}[C] Stay neutral{RESET}        No effect either way.
  {RED}[R] Distance yourself{RESET}   Owner {GOLD}frustrated{RESET}. Small legitimacy signal.
""")
            while True:
                r = prompt("Decision [A/C/R]:").strip().upper()
                if r in ("A","C","R"): break
  ...
  {RED}[S] Suspend & fine heavily{RESET}   Owner suspended 1 season. Legitimacy {GREEN}+8%{RESET}.
                               Owner {RED}furious{RESET}. Strong public message.
  {GOLD}[F] Fine only{RESET}               Public fine, no suspension.
                               Legitimacy {GREEN}+3%{RESET}. Owner {RED}unhappy{RESET}.
  {MUTED}[I] Ignore / bury it{RESET}        Legitimacy {RED}−12%{RESET}. League looks complicit.
""")
            while True:
                r = prompt("Decision [S/F/I]:").strip().upper()
                if r in ("S","F","I"): break
            if r == "S":
  ...
  {RED}[S] Public sanction{RESET}     Required apology + remediation program.
                           Legitimacy {GREEN}+5%{RESET}. Players {GREEN}notice the response{RESET}. Owner {RED}unhappy{RESET}.
  {GOLD}[F] Private reprimand{RESET}   Handle behind closed doors. No immediate effect.
  {MUTED}[I] Ignore{RESET}             Legitimacy {RED}−8%{RESET}. Player morale {RED}−3%{RESET}. League looks complicit.
""")
            while True:
                r = prompt("Decision [S/F/I]:").strip().upper()
                if r in ("S","F","I"): break
  ...
  {RED}[F] Force immediate sale{RESET}      Ownership transition now.
                               Legitimacy {GREEN}+10%{RESET}. Sends a clear message.
  {GOLD}[S] Suspend pending review{RESET}   Owner suspended during investigation.
                               Legitimacy {GREEN}+4%{RESET}. Outcome TBD.
  {MUTED}[I] Ignore{RESET}                  Legitimacy {RED}−15%{RESET}.
                               If fraud is later confirmed: catastrophic.
""")
            while True:
                r = prompt("Decision [F/S/I]:").strip().upper()
                if r in ("F","S","I"): break
            if r == "F":
...
""")
  ...
  ─────────────────────────────────────────────────────────
  {GREEN}[A] Accept{RESET}   {d['accept']}
  {GOLD}[C] Counter{RESET}  {d['counter']}
  {RED}[R] Reject{RESET}   {d['reject']}
  ─────────────────────────────────────────────────────────
  Current work stoppage risk: {risk_c}{stoppage_risk}%{RESET}
""")

            while True:
                raw = prompt("Your decision [A/C/R]:").strip().upper()
                if raw in ("A", "C", "R"):
                    break

            if raw == "A":
                self._cba_apply(league, did, "accept")
                results.append((did, "A", d["accept_s"]))
            elif raw == "C":
                self._cba_apply(league, did, "counter")
                results.append((did, "C", d["counter_s"]))
            else:
                stoppage_risk += d["risk"]
                self._cba_apply(league, did, "reject")
                results.append((did, "R", d["reject_s"]))
